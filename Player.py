...
class Player:
    ...
    def compute_reaction(self):
        """
        Computes the reaction of the player (conventional).

        Returns command for the server (str).
        """
    ...
    def compute_ki_reaction(self):
            ...
            for target in self.see:
                ...
                if object_type == 'b':
                    ...
                    input_tensor = torch.tensor([[[float(torch_obj)], \
                        [float(distance)], [float(direction)], [float(reward)]]])
                    break
        ...
        loss.backward()
        self.optimizer.step()
        print(predicted_value, loss.item())
